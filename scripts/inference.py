...
def get_args():
    ...
    parser.add_argument(
        "--t5_path", default="output/pretrained_models/t5_ckpts", type=str
    )
    parser.add_argument(
        "--tokenizer_path", default="output/pretrained_models/sd-vae-ft-ema", type=str
    )
    parser.add_argument("--txt_file", default="asset/samples.txt", type=str)
    parser.add_argument(
        "--model_path",
        default="output/pretrained_models/PixArt-XL-2-1024x1024.pth",
        type=str,
    )
    parser.add_argument("--bs", default=1, type=int)
    parser.add_argument("--cfg_scale", default=4.5, type=float)
    parser.add_argument("--rescale", default=0.7, type=float)
    parser.add_argument(
        "--sampling_algo",
        default="dpm-solver",
        type=str,
...
def visualize(items, bs, sample_steps, cfg_scale, rescale):
    for chunk in tqdm(list(get_chunks(items, bs)), unit="batch"):
        ...
        with torch.no_grad():
            ...
            if args.sampling_algo == "iddpm":
                ...
                n = len(prompts)
                z = torch.randn(
                    n, 4, latent_size_h, latent_size_w, device=device
                ).repeat(2, 1, 1, 1)
                model_kwargs = dict(
...
if __name__ == "__main__":
    ...
    if args.image_size == 512:
        model = PixArt_XL_2(
            input_size=latent_size, lewei_scale=lewei_scale[args.image_size]
        ).to(device)
    else:
        model = PixArtMS_XL_2(
            input_size=latent_size, lewei_scale=lewei_scale[args.image_size]
    ...
    t5 = T5Embedder(
        device="cuda", local_cache=True, cache_dir=args.t5_path, torch_dtype=torch.float
    )
    ...
    save_root = os.path.join(
        img_save_dir,
        f"{datetime.now().date()}_{args.dataset}_epoch{epoch_name}_step{step_name}_scale{args.cfg_scale}_step{sample_steps}_size{args.image_size}_bs{args.bs}_samp{args.sampling_algo}_seed{seed}",
    )
    os.makedirs(save_root, exist_ok=True)
    visualize(items, args.bs, sample_steps, args.cfg_scale, args.rescale)
