...
class ModelMeanType(enum.Enum):
    ...
    V = enum.auto()  # the model predicts v = sqrt(alpha) * eps - sqrt(1-alpha) * x_t
...
class GaussianDiffusion:
    ...
    def p_mean_variance(
        ...
        if self.model_mean_type == ModelMeanType.V:
            pred_xstart = process_xstart(
                self._predict_xtart_from_v(x_t=x, t=t, v=model_output)
            )
    ...
    def _predict_xtart_from_v(self, x_t, t, v):
    ...
    def p_sample_loop(
        ...
        """
        # TODO rsw need to modify the sampling for v scale as well
