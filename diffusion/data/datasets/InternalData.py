...
class InternalData(Dataset):
    def __init__(
        ...
        **kwargs,
        ...
        image_list_json = (
            image_list_json if isinstance(image_list_json, list) else [image_list_json]
        )
        for json_file in image_list_json:
            ...
            self.img_samples.extend(
                [
                    os.path.join(
                        self.root.replace("InternData", "InternImgs"), item["path"]
                    )
            ...
            self.txt_feat_samples.extend(
                [
                    os.path.join(
                        ...
                        item["path"].split("/")[-1].replace(".png", ".npz"),
                    )
                    for item in meta_data_clean
                ]
            )
            self.vae_feat_samples.extend(
                [
                    os.path.join(
                        self.root,
                        f"img_vae_features/{resolution}resolution/noflip",
                        item["path"].split("/")[-1].replace(".png", ".npy"),
                    )
                    for item in meta_data_clean
                ]
            )
            # TODO rsw the generate vae script and this path do not agree
            # wants something like
            # /home/ubuntu/workspace/pixart-pokemon/img_vae_features_512resolution/noflip/images_400.npy
    ...
    def getdata(self, index):
        ...
        data_info = {
            "img_hw": torch.tensor(
                [
                    self.meta_data_clean[index]["height"],
                    self.meta_data_clean[index]["width"],
                ],
                dtype=torch.float32,
            ),
